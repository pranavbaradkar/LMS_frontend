<template>
    ...
    <v-tabs v-model="activeTab" hide-slider>
      <v-tab v-for="(tab, index) in Array.from(new Set(subjectCategoryNames)) " :key="index">
        <v-chip :class="activeTab === index ? 'm-q-chip': 'm-disabled-chip'" @click="activeTab = index">{{ tab }}</v-chip>
      </v-tab>
